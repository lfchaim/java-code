...
public final class SequenceGenerator {
	...
	private static final SequenceGenerator DEF_GENERATOR = new SequenceGenerator(0L,
			Long.parseLong(PREFS.get("seq_0", "1")));

	static {
		Runtime.getRuntime().addShutdownHook(new Thread(() -> {
			GENERATORS.values().stream().map(SoftReference::get).filter(seq -> seq != null && seq.isPersistOnExit())
	...
	public String toString() {
		return "{" + "counter=" + counter + ", seq=" + sequenceId + '}';
	}
	...
	public static SequenceGenerator create(long initialValue) {
		...
		SequenceGenerator seq = new SequenceGenerator(sequenceId,
				Long.parseLong(PREFS.get("seq_" + sequenceId, "" + initialValue)));
