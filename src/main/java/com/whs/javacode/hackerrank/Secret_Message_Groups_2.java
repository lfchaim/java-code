...
public class Secret_Message_Groups_2
{
	...
	public static void main(String[] args)
	{
		...
		ArrayList<ArrayList<String>> freq = new ArrayList<ArrayList<String>>();
		...
		for (String key : data_base.keySet())
		{
			...
			if (che == 0)
			{
				...
				ArrayList<String> tempp_array_list = new ArrayList<String>();
		...
		ArrayList<String[]> final_print = new ArrayList<String[]>();

		for (int i = 0; i < freq.size(); i++)
		{
			if (freq.get(i).size() == max)
			{
				String[] sortin_array = new String[max];
				for (int j = 0; j < max; j++)
				{
...
//
				for (int i1 = 0; i1 < max; i1++)
				{
					for (int i2 = 0; i2 < max - 1; i2++)
					{
						...
						{
							String sort_tem = sortin_array[i2 + 1];
...
//				}
					...
					{
						if(!bigger(sortin_array[0],final_print.get(i3)[0]))
//						if(final_print.get(i3)[0]>sortin_array[0])
						{
							final_print.add(i3, sortin_array);
							che=1;
							break;
						}
...
//System.out.println(freq);
	...
	public static int[] total_freq(String data)
	{
		int[] ans = new int[10];
		for(int i =0;i<data.length();i++)
		{
			ans[data.charAt(i)-48]++;
		}
