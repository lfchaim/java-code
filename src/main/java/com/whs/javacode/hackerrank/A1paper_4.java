...
//			System.out.println((float)Math.pow(2,(float) (-3)/4));
	...
	{
		if(dim%2==0)
			return (float) Float.parseFloat( String.format("%.12f",(0.5946035575/(float)Math.pow(2, (dim/2)-1))));
		else
			return (float)  Float.parseFloat(String.format("%.12f",(0.42044820762/(float)Math.pow(2, ((dim-1)/2)-1))));
	}
